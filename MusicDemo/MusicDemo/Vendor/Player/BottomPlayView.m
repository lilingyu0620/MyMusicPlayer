...
+ (BottomPlayView *)shareBottomPlayView {
    ...
    dispatch_once(&onceToken, ^{
        bpv = [[BottomPlayView alloc] initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height - 50-64, [UIScreen mainScreen].bounds.size.width, 50)];
    });
    return bpv;
}
...
- (void)setupPlayerWithModel:(MusicModel *)model  {
    ...
    _lblSongName.text = model.name;
    _lblSinger.text = model.singerName;
    if ([_pic_url isEqualToString:@""]) {
        _pic_url = model.picUrl;
    }
    [_songImage setImageWithURL:[NSURL URLWithString:_pic_url] placeholder:[UIImage imageNamed:@"default.jpg"]];
    
    //播放器
    self.playerItem = [AVPlayerItem playerItemWithURL:[NSURL URLWithString:[model.urlList firstObject][@"url"]]];
    //添加通知
    ...
    if (model != nil) {
        ...
        record.songID = [NSString stringWithFormat:@"%@", model.songId];
        record.songFrom = @"playRecord";
        record.songImage = _pic_url;
        record.songName = model.name;
        record.songSinger = model.singerName;
        record.songUrl = [model.urlList firstObject][@"url"];
        record.songPlayCount = [NSString stringWithFormat:@"%ld",(long)model.favorites];
